...
async def _require_portfolio_editor(user: dict) -> None:
    ...
    The portfolio is one company-wide set. Sales reps and marketing may add
    and edit it; admin keeps super-role access. Re-reads role+active (B.2)
    so a demoted or deactivated token cannot widen scope.
    """
    live_role = await authz._live_role(user)
    if not authz.is_portfolio_editor(live_role):
        raise HTTPException(
            status_code=403,
            detail="Sales, marketing, or admin role required to edit the shared portfolio.",
        )
...
async def _require_active_sales_rep(rep_id: str) -> None:
    """404/400 unless rep_id is an active user whose canonical role is sales."""
    rows = await query(
    ...
    if authz.normalize_role(rows[0].get("role")) != "sales":
        raise HTTPException(
            status_code=400,
            detail="rep_id must be an active user with the sales role.",
        )
...
async def _authorize_roster_create(
    ...
    if token_role == "sales" and (rep_id is None or rep_id == caller_id):
        if not caller_id:
