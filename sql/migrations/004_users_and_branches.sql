-- ---------------------------------------------------------------------------
-- Migration 004 — users DDL + branch model (registry + territories) + roles
-- (Handoff 18 — Roles, Permissions & Branch Scoping)
--
-- Apply by hand against the live **crm** DB (see sql/migrations/README.md):
--   mysql -h 127.0.0.1 -P 3306 -u <user> -p crm < sql/migrations/004_users_and_branches.sql
--
-- 1. Commits the real `users` table DDL. The auth path reads
--    `SELECT … FROM users` (api/server.py entra_callback, /api/users) and
--    api/seed_auth.py writes to `users`, but only `crm_users` had committed
--    DDL (sql/crm_users.sql, juniper-prefixed and never applied to crm).
--    `users` is the canonical name; `crm_users` is legacy — if a live
--    `crm_users` table exists with rows and `users` does not, rename it:
--      RENAME TABLE crm_users TO users;   -- run INSTEAD of the CREATE below
--
-- 2. Branch model — TWO tables at two grains (see the branch review):
--    a. `branches` — one row per REAL Aspire branch (BranchID PK), mirrored
--       from the Aspire branches export: name, address, manager, active flag.
--       The canonical registry of physical branches.
--    b. `sales_territories` — the "City, ST" units the app actually operates
--       on: the intake dropdown, estimates.branch, properties.branch_city, and
--       branch read-scoping (api/authz.resolve_branch_name) all speak this
--       vocabulary. Each territory routes to a maintenance + an install Aspire
--       branch (logical refs into branches.aspire_branch_id). Territory→branch
--       is many-to-one and can cross cities (Raleigh+Wilmington→Raleigh;
--       Lancaster→Harrisburg), so it is NOT derivable from branch data alone.
--       ⚠️ authz.resolve_branch_name() reads `sales_territories` (not
--       `branches`) — deploy that code change with this migration.
--
-- 3. Migrates legacy roles onto the canonical 9-role vocabulary:
--    inside_sales/outside_sales → sales. Valid roles (api/authz.py):
--    procurement, sales, admin, manager, regional_director,
--    maintenance_estimating, install_estimating, vice_president, ceo.
--
-- ⚠️ GCP MySQL was unreachable during development — verify existing table
--    names/columns against the live crm DB before applying (esp. step 1).
-- ---------------------------------------------------------------------------

-- 1. users (the table auth actually queries) --------------------------------
CREATE TABLE IF NOT EXISTS users (
    id              VARCHAR(36)  NOT NULL,
    name            VARCHAR(255) NOT NULL,
    email           VARCHAR(255) NOT NULL,
    -- Canonical role (validated in api/authz.py; enum column deliberately
    -- avoided so adding a role is a code deploy, not a DDL migration).
    role            VARCHAR(50)  NOT NULL,
    -- The sales_territories.id the user covers ("Orlando, FL"), NOT an Aspire
    -- BranchID — a rep works a city (both its install + maintenance branches).
    branch_id       VARCHAR(100) NULL DEFAULT NULL,
    avatar_initials VARCHAR(5)   NOT NULL,
    -- Aspire ContactID (~278690-range), written as SalesRepContactID on
    -- opportunity create. Contact-space id, NOT the Aspire UserID.
...
-- 2a. branches — per-branch registry (one row per Aspire BranchID) -----------
...
INSERT INTO sales_territories (id, name, state, aspire_branch_id_maintenance, aspire_branch_id_install) VALUES
...
-- 3. Legacy role migration ----------------------------------------------------
UPDATE users SET role = 'sales' WHERE role IN ('inside_sales', 'outside_sales');

-- Verification: must return 0 rows.
SELECT id, email, role FROM users
 WHERE role NOT IN ('procurement','sales','admin','manager','regional_director',
                    'maintenance_estimating','install_estimating','vice_president','ceo');
