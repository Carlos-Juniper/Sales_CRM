...
- **065** — commission payout installments, the standard plan (rates and payout cadence as data), and an empty billing ledger. Adds `commission_plans`, `commission_plan_rules` (including `payout_schedule`), `user_commission_plans`, `commission_installments`, `commission_billing_events`, nullable `commissions.plan_key` / `commissions.client_type` / `commissions.contract_start_date`, and installment basis columns `billing_installment_number` and `collected_amount_cents`. Maintenance rules use `maintenance_3_payment` (three rows; payment 1 is dated from the contract start or won date, payments 2 and 3 stay `pending_billing_data`). Install rules use `construction_billing_quarterly` and enhancement rules use `enhancement_month_after_quarter` (one row each, null payout date, pending billing). `commission_billing_events` is not seeded. Does not insert per-user plan rows and does not change `commission_rates`. Detector keys on the new tables, `payout_schedule`, `contract_start_date`, the basis columns, both snapshot columns, both unique indexes, the three seeded schedules, and a complete installment-1 backfill.
- **066** — data only. Assigns every commission-earning sales user to `user_commission_plans` plan `standard` with fixed `effective_date` 2026-09-25. Roles are the union `sales`, `outside_sales`, `maintenance_sales`, `install_sales`, `inside_sales` (inside sales is in `SALES_REP_DB_ROLES`), and `vp_sales`, so this file and the later role rename can apply in either order. Michelle Cady and Rodrigo Leon are excluded by case-insensitive trimmed `users.name` or by email containing `cady` / `leon`. Their `commission_rates` rows are not read or written. The runner warns when either person matches zero users or more than one. Idempotent (`INSERT IGNORE` and `NOT EXISTS`). An explicit plan row takes precedence over a legacy rate, so every other sales user moves onto the standard plan. Detector keys on zero eligible users missing that standard row.
