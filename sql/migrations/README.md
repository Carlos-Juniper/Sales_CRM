...
- **067** — data-only: users on `sales` or `outside_sales` become `maintenance_sales`. `vp_sales` is granted only by `scripts/migrate.py` (not by the SQL file) when exactly one of those rows has the exact first and last name Michelle Cady (`Michelle Cady` or `Cady, Michelle`). Email is not a match key. Zero or several matches grant `vp_sales` to nobody and the runner logs an error. Other roles are not updated. Numbered 067 because the commissions PR uses 065 and 066. Detector: the `schema_migrations` row for this migration, not the current set of roles.
