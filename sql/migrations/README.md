...
- **065** — commission payout installments plus the standard plan (rates and payout cadence as data, not code). Adds `commission_plans`, `commission_plan_rules` (including `payout_schedule`), `user_commission_plans`, `commission_installments`, and nullable `commissions.plan_key` / `commissions.client_type`. Maintenance rules use `maintenance_split_lagged` (two installments, one quarter apart, lagging the close quarter). Install rules use `quarter_end` (one installment on the last day of the won quarter). Enhancement rules use `month_after_quarter_end` (one installment on the first day of the month after that quarter). Those install and enhancement dates are a won-date stand-in until billing and collections data exists. Backfill is type-specific and corrects a non-maintenance second installment if one was written earlier. Does not insert per-user plan rows and does not change `commission_rates`. Detector keys on the new tables, `payout_schedule`, both snapshot columns, both unique indexes, the three seeded schedules, and a complete installment-1 backfill.
