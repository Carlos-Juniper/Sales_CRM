...
function readRows<T extends {
  ...
  if (!repId) {
    // Proposal pickers omit rep_id and filter the shared roster by region.
    return rosterHttpResponse(request, staticRows, options)
  }
  const denied = authorizeRead(repId)
  if (denied) return denied
  const visible = rows.filter((row) => row.ownerUserId === repId)
  return rosterHttpResponse(request, visible, options)
}

export const rosterHandlers = [
  http.get(`${API}/proposals/config/team-members`, ({ request }) =>
    readRows(request, teamMemberRows, MOCK_TEAM_MEMBERS, { filterTeamType: true }),
  ),
  http.get(`${API}/proposals/config/client-references`, ({ request }) =>
    readRows(request, clientReferenceRows, MOCK_CLIENT_REFERENCES),
  ),
