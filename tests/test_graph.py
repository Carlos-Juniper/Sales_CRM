...
from api.graph import get_valid_token, send_mail, list_events, create_event  # noqa: E402
...
async def test_get_valid_token_raises_when_no_token_stored():
    """AC-1b: user has never granted Graph scopes → raises ValueError."""
...
async def test_get_valid_token_raises_when_refresh_fails():
    """AC-1d: MSAL refresh returns error dict → raises ValueError."""
    mock_msal_app = MagicMock()
...
async def test_send_mail_posts_to_graph_and_returns_message_id():
    """AC-2a: sends POST to Graph sendMail endpoint, returns message id."""
    mock_client = _make_http_client(
        status=202,
        headers={"x-ms-request-id": "graph-msg-id-001"},
    )
    ...
    assert msg_id == "graph-msg-id-001"
...
async def test_send_mail_raises_502_on_graph_http_error():
    """AC-2b: Graph returns 4xx/5xx → raises HTTPException 502."""
    import httpx
...
async def test_send_mail_includes_cc_when_provided():
    """AC-2c: cc recipients appear in ccRecipients in the request payload."""
    mock_client = _make_http_client(status=202)
    ...
    assert "ccRecipients" in call_json["message"]
    assert call_json["message"]["ccRecipients"][0]["emailAddress"]["address"] == "cc@example.com"
